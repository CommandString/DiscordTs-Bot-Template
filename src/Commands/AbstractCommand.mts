import { ApplicationCommand, ApplicationCommandOptionType, CommandInteraction, Events, SlashCommandBuilder } from 'discord.js';
...
export default class AbstractCommand {
    ...
    public matchesApplicationCommand(command: ApplicationCommand): boolean {
        const thisJson = this.getConfig().toJSON();

        for (const value in thisJson) {
            // @ts-ignore
